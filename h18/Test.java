...
public class Test {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 2 该类能够用一个 .ini文件初始化, .ini文件的格式参考 my_config.ini，测试的时候这个文件名字可能改变，文件中包括中文，且文件一定为
			// UTF-8编码
			// .ini文件中描述了这个类厂能够动态创建的类和类的初始属性
			// 初始化类厂
			// MyClassFactory f=new
			// MyClassFactory("D:\\course\\course\\exam\\src\\main\\java\\com\\huawei\\classroom\\student\\h18\\my_config.txt");
			MyClassFactory f = new MyClassFactory(
					"D:\\Javacode\\my_test\\src\\com\\huawei\\classroom\\student\\h18\\my_config.txt");
			...
			if ("红色".equals(apple.getColor()) && 500 == apple.getWeight()) {
				...
				System.out.println("case 1 wrong!");
